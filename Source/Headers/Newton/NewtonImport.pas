...
{
 ==Notes==
 1. Declare NEWTON_DOUBLE_PRECISION in Projects->Options->Conditionals to Enable Double Precision
 2. NICE_CODE_PARAMS makes code complete and code parameters hint window show the actual type 
    for ex. PNewtonWorld instead of just "Pointer", thus making programming a lot easier.
}

//Defines
 {$DEFINE NICE_CODE_PARAMS} (*Comment this line if you get weird errors*)
//
...
function  NewtonMeshGetVertexIndex (const Mesh : PNewtonMesh; const Vertex : PNewtonMeshVertex) : Int;                                           cdecl; external{$IFDEF __GPC__}name 'NewtonMeshGetVertexIndex'{$ELSE}NewtonDLL{$ENDIF __GPC__};
...
function  NewtonMeshGetNextEdge (const Mesh : PNewtonMesh; const Edge : PNewtonMeshEdge) : PNewtonMeshEdge;                                      cdecl; external{$IFDEF __GPC__}name 'NewtonMeshGetNextEdge'{$ELSE}NewtonDLL{$ENDIF __GPC__};
