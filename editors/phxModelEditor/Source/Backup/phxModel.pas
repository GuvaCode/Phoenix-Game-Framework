...
TPHXMeshTextureList = class
  private
    ...
    procedure SetItem(const Index: Integer; const Value: TPHXMeshTexture);
  ...
  public
    ...
    // The groups in the list
    property Items[const Index: Integer]: TPHXMeshTexture read GetItem write SetItem; default;
  end;
...
// Known layers: 0 - Diffuse 1 - Decal 2 - Specular 3 - Emissive 9 - Normal
//------------------------------------------------------------------------------
TPHXMeshTextures = class
  private
    ...
    FDisplacement: String;
    // Texture containing the team color mask
    FTeam: String;
    // Custom texture #1
    FCustom1: String;
    // Custom texture #2
  ...
  public
    // Diffuse
    property Diffuse: String read FDiffuse write FDiffuse;
    // Decal
    property Decal: String read FDecal write FDecal;
    // Normal
    property Normal: String read FNormal write FNormal;
    // Displacement
    property Displacement: String read FDisplacement write FDisplacement;
    // Texture containing the team color mask
    ...
    //
    property Textures[const Index: Integer]: String read GetTexture write SetTexture; default;
  end;
...
TPHXMeshMaterial = class
  private
    ...
    FTextures2: TPHXMeshTextureList;
...
TPHXMesh = class
  ...
  public
    ...
    procedure Render(Buffer: TPHXBuffer); overload;
    // Render a single group
    procedure RenderGroup(Buffer: TPHXBuffer; const Group: Integer); overload;
    // Render the normals of the mesh
    procedure RenderNormals(Buffer: TPHXBuffer; const Length: Single; const Color: TColor4f);


    // Transform the model by an matrix
    procedure Transform(const Matrix: TMatrix4);
...
begin
  ...
  begin
    ...
    Reader.Read(FList^[Index].Flags       , SizeOf(FList^[Index].Flags));
...
begin
  ...
  begin
    ...
    Writer.Write(FList^[Index].Flags       , SizeOf(FList^[Index].Flags));
...
// TPHXMeshTextures
//==============================================================================
constructor TPHXMeshTextureList.Create(AMesh: TPHXMesh);
begin
  FMesh    := AMesh;
  FCount   := 0;
  FCapacity:= 0;
...
//------------------------------------------------------------------------------
destructor TPHXMeshTextureList.Destroy;
begin
  SetCapacity(0);
end;

//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.LoadTextures(Reader: TPHXReader);
var Index: Integer;
begin
  FCount:= Reader.ReadInteger;
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.SaveTextures(Writer: TPHXWriter);
var Index: Integer;
begin
  Writer.WriteInteger(FCount);
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.Clear;
begin
  FCount:= 0;
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.Add(const Value: TPHXMeshTexture);
begin
  Inc(FCount);
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.Add(const Name: String; const Kind: TPHXMeshTextureKind);
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.Grow;
var Delta: Integer;
begin
  if FCapacity > 64 then
  begin
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.SetCapacity(const Value: Integer);
begin
  if FCapacity <> Value then
  begin
    FCapacity := Value;
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.SetCount(const Value: Integer);
begin
  FCount := Value;
...
//------------------------------------------------------------------------------
function TPHXMeshTextureList.GetItem(const Index: Integer): TPHXMeshTexture;
begin
  Assert( (Index >= 0) and (Index < Count), 'List index out of bounds');
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextureList.SetItem(const Index: Integer; const Value: TPHXMeshTexture);
begin
  Assert( (Index >= 0) and (Index < Count), 'List index out of bounds');
...
//------------------------------------------------------------------------------
function TPHXMeshTextures.GetTexture(const Index: Integer): String;
begin
  Result:= '';
  case Index of
    0: Result:= FDiffuse;
...
//------------------------------------------------------------------------------
procedure TPHXMeshTextures.SetTexture(const Index: Integer; const Value: String);
begin
  case Index of
    0: FDiffuse:= Value;
    1: FDecal:= Value;
    2: FNormal:= Value;
    3: FDisplacement:= Value;
    4: FTeam:= Value;
    5: FCustom1:= Value;
    6: FCustom2:= Value;
    7: FCustom3:= Value;
...
begin
  FTextures2:= TPHXMeshTextureList.Create(AMesh);

  FName    := 'Material';
  FColor   := clrWhite;
  FFlags   := 0;
  FShader  := '';
  FTextures:= TPHXMeshTextures.Create;
end;
...
begin
  ...
  FTextures2.LoadTextures(Reader);
end;
...
begin
  ...
  FTextures2.SaveTextures(Writer);
end;

//------------------------------------------------------------------------------
procedure TPHXMeshMaterial.LoadFromFile(const FileName: String);
var Stream: TStream;
begin
...
//------------------------------------------------------------------------------
procedure TPHXMeshMaterial.SaveToFile(const FileName: String);
var Stream: TStream;
begin
...
//------------------------------------------------------------------------------
procedure TPHXMeshMaterial.LoadFromStream(Stream: TStream);
begin
  ...
  FShader               := ReadString(Stream);
  FTextures.Diffuse     := ReadString(Stream);
  FTextures.Decal       := ReadString(Stream);
  FTextures.Normal      := ReadString(Stream);
  FTextures.Displacement:= ReadString(Stream);
  FTextures.Team        := ReadString(Stream);
  FTextures.Custom1     := ReadString(Stream);
  FTextures.Custom2     := ReadString(Stream);
  FTextures.Custom3     := ReadString(Stream);
end;



//------------------------------------------------------------------------------
procedure TPHXMeshMaterial.SaveToStream(Stream: TStream);
begin
  ...
  WriteString(Stream, FShader);

  WriteString(Stream, FTextures.Diffuse);
  WriteString(Stream, FTextures.Decal);
  WriteString(Stream, FTextures.Normal);
  WriteString(Stream, FTextures.Displacement);
  WriteString(Stream, FTextures.Team);
  WriteString(Stream, FTextures.Custom1);
  WriteString(Stream, FTextures.Custom2);
  WriteString(Stream, FTextures.Custom3);
end;
...
//------------------------------------------------------------------------------
procedure TPHXMesh.LoadFromStreamEx(Stream: TStream);
var Header: TPHXMeshHeader;
var Index : Integer;
var Count : Integer;
begin
  ...
  ReadString(Stream, FComment);
  ReadString(Stream, FAuthor);
  ReadString(Stream, FVersion);
  // Load the material
  Material.LoadFromStream(Stream);

  // Read verticies
  Stream.Read(Count, SizeOf(Count)); Vertices.Count:= Count;
  for Index := 0 to Count - 1 do
  begin
    Stream.Read( Vertices.List^[Index].Joint    , SizeOf(Vertices.List^[Index].Joint));
    Stream.Read( Vertices.List^[Index].Position , SizeOf(Vertices.List^[Index].Position));
    Stream.Read( Vertices.List^[Index].Normal   , SizeOf(Vertices.List^[Index].Normal));
    Stream.Read( Vertices.List^[Index].Tangent  , SizeOf(Vertices.List^[Index].Tangent));
    Stream.Read( Vertices.List^[Index].TexCoord0, SizeOf(Vertices.List^[Index].TexCoord0));
    Stream.Read( Vertices.List^[Index].TexCoord1, SizeOf(Vertices.List^[Index].TexCoord1));
  end;
  // Read triangles
  Stream.Read(Count, SizeOf(Count)); Triangles.Count:= Count;
  for Index := 0 to Count - 1 do
  begin
    Stream.Read( Triangles.List^[Index].Vertex0, SizeOf(Triangles.List^[Index].Vertex0));
    Stream.Read( Triangles.List^[Index].Vertex1, SizeOf(Triangles.List^[Index].Vertex1));
    Stream.Read( Triangles.List^[Index].Vertex2, SizeOf(Triangles.List^[Index].Vertex2));
  end;
  // Read groups
  Stream.Read(Count, SizeOf(Count)); Groups.Count:= Count;
  for Index := 0 to Count - 1 do
  begin
    Stream.Read( Groups.List^[Index].Name        , SizeOf(Groups.List^[Index].Name));
    Stream.Read( Groups.List^[Index].Flags       , SizeOf(Groups.List^[Index].Flags));

    Stream.Read( Groups.List^[Index].TriangleOffset, SizeOf(Groups.List^[Index].TriangleOffset));
    Stream.Read( Groups.List^[Index].TriangleCount , SizeOf(Groups.List^[Index].TriangleCount));
  end;
  // Read joints
  Stream.Read(Count, SizeOf(Count)); Joints.Count:= Count;
  for Index := 0 to Count - 1 do
  begin
    Stream.Read( Joints.List^[Index].Name       , SizeOf(Joints.List^[Index].Name));
    Stream.Read( Joints.List^[Index].Parent     , SizeOf(Joints.List^[Index].Parent));
    Stream.Read( Joints.List^[Index].ParentName , SizeOf(Joints.List^[Index].ParentName));
    Stream.Read( Joints.List^[Index].Position   , SizeOf(Joints.List^[Index].Position));
    Stream.Read( Joints.List^[Index].Rotation   , SizeOf(Joints.List^[Index].Rotation));
  end;
  // Read Tags
  Stream.Read(Count, SizeOf(Count)); Tags.Count:= Count;
  for Index := 0 to Count - 1 do
  begin
    Stream.Read( Tags.List^[Index].Name       , SizeOf(Tags.List^[Index].Name));
    Stream.Read( Tags.List^[Index].Joint      , SizeOf(Tags.List^[Index].Joint));
    Stream.Read( Tags.List^[Index].JointName  , SizeOf(Tags.List^[Index].JointName));
    Stream.Read( Tags.List^[Index].Position   , SizeOf(Tags.List^[Index].Position));
    Stream.Read( Tags.List^[Index].Direction  , SizeOf(Tags.List^[Index].Direction));
  end;
...
//------------------------------------------------------------------------------
procedure TPHXMesh.SaveToStreamEx(Stream: TStream);
var Header: TPHXMeshHeader;
var Index : Integer;
var Count : Integer;
begin
  ...
  // Write meta data
  WriteString(Stream, FName);
  WriteString(Stream, FComment);
  WriteString(Stream, FAuthor);
  WriteString(Stream, FVersion);
  // Save the material
  Material.SaveToStream(Stream);
  ...
  for Index := 0 to Count - 1 do
  begin
    Stream.Write( Vertices.List^[Index].Joint    , SizeOf(Vertices.List^[Index].Joint));
    Stream.Write( Vertices.List^[Index].Position , SizeOf(Vertices.List^[Index].Position));
    Stream.Write( Vertices.List^[Index].Normal   , SizeOf(Vertices.List^[Index].Normal));
    Stream.Write( Vertices.List^[Index].Tangent  , SizeOf(Vertices.List^[Index].Tangent));
    Stream.Write( Vertices.List^[Index].TexCoord0, SizeOf(Vertices.List^[Index].TexCoord0));
    Stream.Write( Vertices.List^[Index].TexCoord1, SizeOf(Vertices.List^[Index].TexCoord1));
  end;
  // Write triangles
  Count:= Triangles.Count; Stream.Write(Count, SizeOf(Count));
  for Index := 0 to Count - 1 do
  begin
    Stream.Write( Triangles.List^[Index].Vertex0, SizeOf(Triangles.List^[Index].Vertex0));
    Stream.Write( Triangles.List^[Index].Vertex1, SizeOf(Triangles.List^[Index].Vertex1));
    Stream.Write( Triangles.List^[Index].Vertex2, SizeOf(Triangles.List^[Index].Vertex2));
  end;
  // Write groups
  Count:= Groups.Count;
  ...
  for Index := 0 to Count - 1 do
  begin
    Stream.Write( Groups.List^[Index].Name        , SizeOf(Groups.List^[Index].Name));
    Stream.Write( Groups.List^[Index].Flags       , SizeOf(Groups.List^[Index].Flags));

    Stream.Write( Groups.List^[Index].TriangleOffset, SizeOf(Groups.List^[Index].TriangleOffset));
    Stream.Write( Groups.List^[Index].TriangleCount , SizeOf(Groups.List^[Index].TriangleCount));
  end;
  // Write joints
  Count:= Joints.Count;
  ...
  for Index := 0 to Count - 1 do
  begin
    Stream.Write( Joints.List^[Index].Name       , SizeOf(Joints.List^[Index].Name));
    Stream.Write( Joints.List^[Index].Parent     , SizeOf(Joints.List^[Index].Parent));
    Stream.Write( Joints.List^[Index].ParentName , SizeOf(Joints.List^[Index].ParentName));
    Stream.Write( Joints.List^[Index].Position   , SizeOf(Joints.List^[Index].Position));
    Stream.Write( Joints.List^[Index].Rotation   , SizeOf(Joints.List^[Index].Rotation));
  end;
  // Write Tags
  Count:= Tags.Count;
  ...
  for Index := 0 to Count - 1 do
  begin
    Stream.Write( Tags.List^[Index].Name       , SizeOf(Tags.List^[Index].Name));
    Stream.Write( Tags.List^[Index].Joint      , SizeOf(Tags.List^[Index].Joint));
    Stream.Write( Tags.List^[Index].JointName  , SizeOf(Tags.List^[Index].JointName));
    Stream.Write( Tags.List^[Index].Position   , SizeOf(Tags.List^[Index].Position));
    Stream.Write( Tags.List^[Index].Direction  , SizeOf(Tags.List^[Index].Direction));
  end;
...
//------------------------------------------------------------------------------
procedure TPHXMesh.Transform(const Matrix: TMatrix4);
var Index: Integer;
begin
...
//------------------------------------------------------------------------------
procedure TPHXMesh.Render(Buffer: TPHXBuffer);
var Index: Integer;
var IV   : Integer;
var II   : Integer;
begin
...
//------------------------------------------------------------------------------
procedure TPHXMesh.RenderGroup(Buffer: TPHXBuffer; const Group: Integer);
var Index: Integer;
var IV   : Integer;
var II   : Integer;
begin
...
//------------------------------------------------------------------------------
procedure TPHXMesh.RenderNormals(Buffer: TPHXBuffer; const Length: Single; const Color: TColor4f);
var Index: Integer;
var IV   : Integer;
var II   : Integer;
begin
